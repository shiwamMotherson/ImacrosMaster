...
public class AdvancedSftpIteration {
    ...
    public static void main(String args[]) throws IOException {
		...
		Instant yesterday = now.minus(r, ChronoUnit.DAYS);
		System.out.println();
		String yesterday1=yesterday.toString();
		System.out.println(yesterday1);
 	   File directoryPath = new File("D:\\imacro\\"+imformat(Instant.now().minus(s, ChronoUnit.DAYS).toString()));

 	   AdvancedSftpIteration sftpFileTransfer=new AdvancedSftpIteration();
 	   sftpFileTransfer.createRootDirectory(directoryPath,imformat(yesterday1));
    	}
    	}
    }
    public void createRootDirectory(File directoryPath,String directoryPathtoday) {
        System.out.println(directoryPath+"/------//------/stp upload called"+directoryPathtoday);
        ...
        try {
			...
			//}  
            createChildDirectory(channelSftp,directoryPath,directoryPathtoday);
            channelSftp.exit();
    ...
    public void createChildDirectory(ChannelSftp channelSftp,File directoryPath,String directoryPathtoday) {
		...
		if(filesList!=null){	 
    	  for(File file : filesList) {
    		  if(file.isDirectory()) {
    			...
    			//  }  
				  createChildDirectory(channelSftp,file,directoryPathtoday);
			  }else if(file.isFile()) {
				    ...
				    try {
					    ...
					    if(e.id == ChannelSftp.SSH_FX_NO_SUCH_FILE){
					    	...
					    	try {
								...
								String date1x=directoryPathtoday;
								date1x = date1x.substring(4)+"-"+date1x.substring(2,4)+"-"+date1x.substring(0,2)+" 00:00:00.000";
								String fnamex="";
								...
								List<String>invL2=dbt2.getInvoiceNameforsftp(unitx,date1x);
								...
								String date1=directoryPathtoday;
								//String date= filepath.substring(1,9);
								date1 = date1.substring(4)+"-"+date1.substring(2,4)+"-"+date1.substring(0,2)+" 00:00:00.000";
								String fname="";
					...
					//String fname=filepath.substring(26,37);//08042021E01InvoicesGSBSB21-00060.pdf /24032021/E01/Invoices/BP/INB20-01188.pdf
							...
							dbt.updateSFTPUploadStatus(dbt.getUnitConfig(uc,ucat), date1,"Y", fname);
							//upload pdf file
						String filename1 = fname.replace(".pdf","");
							...
							System.out.println("File not Uploaded:"+filepath);
								}
